...
import logging
#log configuration
logging.basicConfig(filename='RedFree.log', level=logging.DEBUG, 
...
class MyFrame(wx.Frame):
    def __init__(self, parent, id, title):
        ...
        check_updates_item = tools_menu.Append(wx.ID_ANY, "Check updates", "Check for updates and close RedFree")
...
# Execute the command in a separate thread
    ...
    def open_github_repo(self, event):
        # GitHub repo URL
        github_url = "https://github.com/RedXUK/RedFree-Windows-Optimizer"
        webbrowser.open(github_url)

    def download_latest_github(self, event):
        # URL to download the latest version from GitHub
        download_url = "https://github.com/RedXUK/RedFree-Windows-Optimizer/releases"
...
def show_welcome_dialog():
    ...
    if not os.path.exists("welcome_indicator"):
        ...
        app = wx.App()
        dlg = WelcomeDialog(None, -1, "Welcome to RedFree")
        result = dlg.ShowModal()
        dlg.Destroy()
        app.MainLoop()
...
def main():
    show_welcome_dialog()
    app = wx.App(False)
    frame = MyFrame(None, -1, "RedFree, Windows Optimizer™")
    frame.Show()
    app.MainLoop()
