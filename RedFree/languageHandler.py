...
def getAvailableLanguages():
    ...
    l=[x for x in os.listdir('locales') if os.path.isfile(os.path.join('locales', x, 'LC_MESSAGES', 'RedFree.mo'))]
...
def setLanguage(lang):
    ...
    try:
        if lang=="system":
            ...
            trans=gettext.translation('RedFree', localedir='locales', languages=[localeName])
            curLang=localeName
        else:
            trans=gettext.translation('RedFree', localedir='locales', languages=[lang])
            curLang=lang
            localeChanged=False
            if system == "Windows":
        ...
        trans=gettext.translation('RedFree', fallback=True)
        curLang="pt"
    if sys.version[0] == "3":
        trans.install()
    else:
