...
class AtualizadorApp(wx.App):
    ...
    def ObterUltimaVersaoGitHub(self):
        url = "https://api.github.com/repos/RedXUK/RedFree-Windows-Optimizer/releases/latest"
        response = requests.get(url)
        if response.status_code == 200:
            data = response.json()
            return data.get("tag_name", "RedFree0")
        return "RedFree0"
    ...
    def AtualizarApp(self, ultima_versao_github):
        install_url = f"https://github.com/RedXUK/RedFree-Windows-Optimizer/releases/download/{ultima_versao_github}/RedFree-install.exe"
        install_file = "RedFree-install.exe"
